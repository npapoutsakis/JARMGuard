...
# Caleb Yu
# 
# Threaded Implementation of JARM by:
...
"""
    Worker function for threads to process a queue item and store results
"""
def thread_worker(jarm_details, results, index, ip_results):

    payload = packet_building(jarm_details)
    ...
    if server_hello == "TIMEOUT":
        ...
        results[index] = read_packet(server_hello, jarm_details)
...
def main():
    ...
    results = ["" for _ in range(len(queue))]
    ip_results = ["" for _ in range(len(queue))]

    threads = []
    for i, req in enumerate(queue):
        thread = threading.Thread(target=thread_worker, args=(req, results, i, ip_results))
    ...
    jarm = ",".join(results)
    if "|||" in jarm and len(jarm.split(",")) != len(queue):
        jarm = "|||,|||,|||,|||,|||,|||,|||,|||,|||,|||"
    

    ip = None
    for ip_addr in ip_results:
